...
ItemType UnsortedType::Head() const {
    if (listData == NULL) throw std::runtime_error("List is empty");
    return listData->info; // O(1)
}

ItemType UnsortedType::Tail() const {
    if (listData == NULL) throw std::runtime_error("List is empty");
    NodeType* location = listData;
    while (location->next != NULL)
        location = location->next;
    return location->info; // O(n)
}
